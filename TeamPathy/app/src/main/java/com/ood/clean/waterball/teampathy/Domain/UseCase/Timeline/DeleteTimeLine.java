...
@ProjectScope
public class DeleteTimeLine extends UseCase<Timeline,Timeline> {
    private TimeLineRepository timeLineRepository;

    @Inject
    public DeleteTimeLine(ThreadingObservableFactory threadingObservableFactory, TimeLineRepository timeLineRepository) {
    ...
    protected Observable<Timeline> buildUseCaseObservable(final Timeline timeline) {
        return Observable.create(new ObservableOnSubscribe<Timeline>() {
            ...
            public void subscribe(@NonNull ObservableEmitter<Timeline> e) throws Exception {
                e.onNext(timeLineRepository.delete(timeline));
                e.onComplete();
            }
        });
    }
